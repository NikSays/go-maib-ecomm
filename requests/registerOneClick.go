...
const (
	// RegisterOneClickSMS is a oneClick transaction type which is initialized with
	// an SMS transaction (-z). The resulting transaction should be confirmed with
	// TransactionStatus (-c).
	//
	// This is the default transaction type.
	RegisterOneClickSMS RegisterOneClickType = iota

	// RegisterOneClickWithoutPayment is a oneClick transaction type which is
	// initialized without a transaction (-p).
	RegisterOneClickWithoutPayment
)

// String converts RegisterOneClickType into the ECommerce command. Returns an
...
// to save a card, like with recurring transactions. However, oneClick
// transactions cannot be instantly executed. The client must be redirected to
// the client handler to confirm the transaction.
type RegisterOneClick struct {
	...
	// Transaction payment amount. Positive integer with last 2 digits being the
	// cents. Ignored for registration without first payment.
	//
	...
	// Identifier of the oneClick payment. If not specified, resulting
	// TRANSACTION_ID will be used as the oneClick payment ID.
	...
	// Whether the oneClick transaction with a given BillerClientID should be
	// updated. This way, same BillerClientID may be used when customer changes
	// payment information.
	OverwriteExisting bool `url:"-"`

	// If true, there will be a checkbox on the client handler. The card will be
	// saved only if the checkbox is checked. The field
	// TransactionStatusResult.RecurringPaymentID will be set only if the card is
	// saved.
